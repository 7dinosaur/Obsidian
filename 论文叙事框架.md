...
> **"大载客量 SST 能不能低声爆？如果能，路线在哪？如果不能，物理底线在哪？——本文用一次系统推演，给出了这个行业悬而未决问题的第一个定量答案。"**
...
3. **超声速客机运营经济学** → 盈亏平衡点 ~150 座（引用 Concorde 商业失败教训：100 座不可持续）
4. **关键冲突浮现**：
   - 低声爆要求等效截面积分布光滑细长（Seebass-George 理论）
   - 大载客量要求等效截面积在客舱段膨胀
   - 两者在**同一条等效截面积曲线上**打架 → 要么牺牲座位，要么牺牲声爆，现有范式下两难
...
| 章节 | 叙事功能 | 核心问题 | 关键图表 |
|------|----------|----------|----------|
| **Introduction** | 立案 | 为什么"大载客量 vs 低声爆"的矛盾值得追问？| 载客量-PLdB 关系示意图 |
...
|------|--------|------|
| X-59 | 1 座 | ~75 |
| JAXA 低音爆概念 | ~40 座 | ~85（若已知） |
| Concorde | 100 座 | ~105 |
| **本文 SBWB** | **160 座** | **~99** |

**趋势线向上**。本文的 99 PLdB 恰好落在趋势线上。

**图注文字要点**：
> "载客量与最低可达声爆水平之间存在近似对数关系。160 座级构型受体积约束，其声爆底线天然高于小型验证机。本文 99 PLdB 符合该趋势——它不是反常的高，而是该类别的物理必然。"

**这张图的长期价值**：将来任何人讨论"XX 座级 SST 能不能做到 YY PLdB"，都会引用这张图。它定义了一个参考框架。
...
2. **读完 Results，读者能否复述一个简明的论证："因为 A 原因，遇到了 B 限制，所以结果是 C"？**
3. **结论中是否有可被引用的数字或关系？（99 PLdB × 160 座这个对应关系应成为行业的引用锚点）**
